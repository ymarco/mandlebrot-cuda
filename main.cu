...
int main(int argc, char* argv[]){
	...
	if(argc != 4 && argc != 6){
		std::cerr << "incorrect amount of arguments.\nusage: " << argv[0] << " center_x center_y zoom [img_width, image_height]\n";
		return -1;
	}
	...
	//std::cout << rerange(0,0) << "," <<rerange(RES_WIDTH, RES_HEIGHT) << ",center = " << rerange(RES_WIDTH/2, RES_HEIGHT/2) << std::endl;
	int IMG_SIZE = img_info.RES_WIDTH * img_info.RES_HEIGHT * sizeof(BmpSave::Color);
	...
	BmpSave::Color* h_img = new BmpSave::Color[img_info.RES_HEIGHT * img_info.RES_WIDTH];
	...
	+ std::to_string(img_info.CENTER_Y) +",zoom="+ std::to_string(img_info.ZOOM) + ".bmp";
